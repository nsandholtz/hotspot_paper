...
get_coverage_UCB = function(par_index_, scale_val_sim_, n_dat){
  ...
  scale_range = full_fits %>%
    ...
    pull(scale_val) %>%
    range()
  cover_par = between(par_vals[[3]][par_index_], par_range[1], par_range[2])
...
#get_coverage_UCB(15, scale_val_sim, n_dat = 100)
...
coverage_UCB <- foreach(
  i = 1:n_sim,
  .combine = 'rbind',
  .packages = c("tidyverse", "data.table")
) %dopar% {
  get_coverage_UCB(par_index, scale_val_sim, n_dat)
}

parallel::stopCluster(cl = my.cluster)

saveRDS(coverage_UCB, file = paste0("./analysis/supp_material/simulation_output/coverage_UCB_",
                                   par_index,"_",
                                   n_dat,
                                   ".rds"))
