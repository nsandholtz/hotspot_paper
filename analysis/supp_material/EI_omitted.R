...
# simulation_coverage_par = matrix(NA, n_sim, length(n_dat)) 
# simulation_coverage_scale = matrix(NA, n_sim, length(n_dat)) 

min_r1 = range(r1_grid)[1]
max_r1 = range(r1_grid)[2]

get_coverage_EI_omit = function(par_index_, scale_val_sim_, n_dat){
  ...
  observed_angles = suppressWarnings(circular::rwrappedcauchy(
    ...
    ifelse(. > pi, . - 2 * pi, .)
  plot(r1_vals, observed_angles, col = which_mode, pch = 16)
  ...
  UCB_in_CI = full_fits %>%
    slice(-c((min(which(cum_post_prob >= .95))+1):n())) %>%
    with(if_else("UCB" %in% unique(acq_type), 1, 0))
  return(c(PI_in_CI, UCB_in_CI))
}
...
EI_omitted <- foreach(
  ...
  get_coverage_EI_omit(par_index, scale_val_sim, n_dat)
}

parallel::stopCluster(cl = my.cluster)

print(apply(EI_omitted, 2, mean, na.rm = T)) 



saveRDS(EI_omitted, file = paste0("./analysis/supp_material/simulation_output/EI_omitted_",
                                   par_index,"_",
                                   n_dat,
                                   ".rds"))
