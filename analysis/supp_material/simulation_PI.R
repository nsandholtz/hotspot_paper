...
get_coverage_PI = function(par_index_, scale_val_sim_, n_dat){
  ...
  # Make inference ----------------------------------------------------------
    ...
    par_range = full_fits %>%
      ...
      pull(par_val) %>%
      range()
    scale_range = full_fits %>%
      slice(-c((min(which(cum_post_prob >= .95))+1):n())) %>%
      pull(scale_val) %>%
      range()
    cover_par = between(par_vals[[1]][par_index_], par_range[1], par_range[2])
    cover_scale = between(scale_val_sim_, scale_range[1], scale_range[2])
    return(c(cover_par, cover_scale))
}


# Do simulation -----------------------------------------------------------

# get_coverage_PI(15, scale_val_sim, n_dat = 100)
...
coverage_PI <- foreach(
  ...
  get_coverage_PI(par_index, scale_val_sim, n_dat)
}

parallel::stopCluster(cl = my.cluster)

saveRDS(coverage_PI, file = paste0("./analysis/supp_material/simulation_output/coverage_PI_",
                                   par_index,"_",
                                   n_dat,
                                  ".rds"))
