...
for(my_sub in c(21, 24, 28)) {
  ...
  posterior_sample_indices = sample(
    1:nrow(acquisition_fits[[my_sub]]$posterior),
    size = 100000,
    prob = acquisition_fits[[my_sub]]$posterior$marg_post_norm,
    replace = T
  )
  num_curves = ifelse(max(posterior_sample_indices) > 2,
                      max(posterior_sample_indices),
                      3)
  ...
  # PLOT
  acq_type_ind = ifelse(
    acquisition_fits[[my_sub]]$MAP$acq_type == "PI",
    1,
  ...
  title_quote = c(
    bquote(.(title_help[acq_type_ind]) ~ "," ~ xi["PI"] ~ " = " ~ .(
      round(acquisition_fits[[my_sub]]$MAP$par_val, digits = 2)
    )),
  ...
  # TEXT
  title(
    paste("Subject", my_sub, "MAP Acquisition Curve"),
    line = .5,
    cex.main = 1
  )
  ...
  lines(
    ...
    col = ifelse(
      ...
      model_cols[1],
      ifelse(
        acquisition_fits[[my_sub]]$MAP$acq_type == "EI",
        model_cols[2],
        model_cols[3]
      )
    )
