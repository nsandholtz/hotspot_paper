...
for(my_sub in unique(ok_event_dat$alt_id)) {
  ...
  acquisition_fits[[my_sub]]$w_est = mean(prob_new)
  ...
  oos_log_lik = round(sym_wc_log_lik4(data_vec = targets[-training],
                  ...
                  scale = acquisition_fits[[my_sub]]$MAP$scale_val,
                  weights = rep(acquisition_fits[[my_sub]]$w_est, length(rewards_1[-training]))), digits = 2)
