...
sym_wc_log_lik = function(data_vec, 
                                  peak_vec, 
                                  scale){
  sum(log(.5* ((1/(2*pi)) * sinh(scale) /(cosh(scale) - cos(abs(data_vec) - peak_vec))) + 
            .5* ((1/(2*pi)) * sinh(scale) /(cosh(scale) - cos(-1*abs(data_vec) + peak_vec)))))   
}
...
sym_wc_log_lik_over_grid = function(subject_rewards,
  ...
  for(i in 1:par_l){
    ...
    for(j in 1:length(scale_vals_)){
      ...
      output[scale_l * (i-1) + j, 3] = sym_wc_log_lik(data_vec = subject_targets,
